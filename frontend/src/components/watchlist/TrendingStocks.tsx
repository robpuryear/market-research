...
function SentimentDot({ value }: { value: number }) {
  ...
  const color = value >= 0.3 ? "text-green-600" : value <= -0.3 ? "text-red-500" : "text-gray-400";
...
export function TrendingStocks() {
  ...
  if (loading) return (
    <div className="bg-white border border-gray-300 rounded-lg p-4">
      ...
      <div className="text-xs text-gray-400 animate-pulse">Scanning Reddit + volume data…</div>
    </div>
  );
    ...
    <div className="bg-white border border-gray-300 rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-gray-400">
          Reddit buzz + volume spike · sorted by momentum score
        </span>
      </div>
        ...
        <table className="w-full text-xs font-mono">
          <thead className="border-b border-gray-200">
            <tr>
              ...
              <th className="text-left py-2 px-3 text-gray-500 uppercase tracking-wider">Reddit</th>
            ...
            {stocks.map((s) => (
              <tr key={s.ticker} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="py-2 px-3">
                  ...
                  <div className="text-gray-400 text-xs truncate max-w-[120px]">{s.company_name}</div>
                </td>
                ...
                <td className="py-2 px-3 text-gray-600">{s.reddit_mentions} mentions</td>
                <td className="py-2 px-3"><SentimentDot value={s.reddit_sentiment} /></td>
                ...
                <td className="py-2 px-3 text-right">
                  {s.on_watchlist ? (
                    ...
                    <button
                      ...
                      className="text-blue-600 hover:text-blue-800 disabled:text-gray-300 text-xs font-semibold transition-colors"
                    >
